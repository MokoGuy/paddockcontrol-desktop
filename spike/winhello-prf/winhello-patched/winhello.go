...
func GetAssertion(
	...
	// LOCAL FIX (go-ctap/winhello v0.1.0): Windows Hello returns the assertion
	// hmac-secret in the native pHmacSecret field (resp.hmacSecret), NOT the CBOR
	// authData extension that the block below checks. Populate the public outputs
	// from the native field so platform-authenticator PRF/hmac-secret works.
	if resp.hmacSecret != nil {
		if extInputs != nil && extInputs.GetHMACSecretInputs != nil {
		...
		if extInputs != nil && extInputs.PRFInputs != nil {
